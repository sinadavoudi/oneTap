...
```bash
./onetap.sh
# Choose: 1 (Quick Setup)
# Choose: 1 (Auto Mode)
# Copy the config link
# Import to v2rayNG/v2rayN
# Connect!
```

### For Iran/China Users (Best Stealth)

```bash
# Option A: Reality Protocol (No domain needed)
./onetap.sh
# Choose: 6 (AnyTLS/Reality)

# Option B: CDN + Clean IPs (Requires domain)
./onetap.sh
# Choose: 7 (CDN Setup)
# Then: 12 (CF Clean IP Scanner)
# Use clean IP in your client
```
...
**Best options:**
1. Option 7 (CDN) + Option 12 (Scanner) - Most reliable
2. Option 6 (Reality) - No domain needed
3. Option 4 (DNSTT) - Backup method

**VPS Location:** Germany, Netherlands, Finland

### 🇨🇳 China

**Best options:**
1. Option 6 (Reality) - Bypasses GFW
2. Option 7 (CDN) + Clean IPs
3. Avoid simple protocols

**VPS Location:** Hong Kong, Japan, Singapore
...
```bash
Run Option 1 (Quick Setup)
Use Auto Mode
Test if it works
If good → Done!
If slow → Enable BBR (Option 9)
```

### Scenario 2: Iran User with Domain
```bash
Run Option 7 (CDN Setup)
Run Option 12 (CF Scanner)
Copy best IP from results
Update client with clean IP
Rescan weekly
```

### Scenario 3: No Domain, Maximum Stealth
```bash
Run Option 6 (Reality)
Use Auto Mode
Connect and test
Enable BBR for speed
```

### Scenario 4: Testing Multiple Protocols
```bash
Run Option 1 (Quick)
Run Option 6 (Reality)
Use Option 11 to switch between them
